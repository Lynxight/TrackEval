...
#     parser.add_argument("--pool-alias",
#                         help="pool alias to evaluate, \n"
#                              "contains pool directory for evaluation to be run",
#                         required=True)
#
#     args = parser.parse_args()
#
#     return TrackEvalArgs(poolAlias=args.pool_alias
#                          )


if __name__ == '__main__':
    #pool_alias = args.poolAlias
    pool_alias = "elisha"
    freeze_support()
    ...
    script_config = {
        ...
        'GT_FOLDER': os.path.join(code_path, f'data/{pool_alias}/gt/rob_mots'),
        'TRACKERS_FOLDER': os.path.join(code_path, f'data/{pool_alias}/trackers/rob_mots'),
    }
    ...
    if not config['BENCHMARKS']:
        if config['ROBMOTS_SPLIT'] == 'val':
            ...
            config['BENCHMARKS'] = [
                'mots_challenge']  # 'kitti_mots', 'davis_unsupervised', 'youtube_vis', 'ovis', 'tao', 'bdd_mots'
    ...
    if success:
        ...
        for tracker in trackers:
            ...
            out_file = os.path.join(config['TRACKERS_FOLDER'], config['ROBMOTS_SPLIT'], tracker,
                                    f'{pool_alias}_final_results.csv')
